...
export class ScheduleByCandidates extends BaseComponent implements OnInit {
  ...
  /*
  generateCandidate(input: AdhiveshanInput): void {
    ...
    //this.input = input;
    length = this.input.totalCandidate!;
    ...
    this.input.skills!.forEach(skill=> {
      for (let p = 0; p < skill.noOfParticipants!; p++) {
        for (let index = 0; index < this.candidates.length; index++) {
          const candidate = this.candidates[index];

          if(candidate.participatingSkills.filter(c=>c.name == skill.name).length == 0)
          {
            if(skill.name == "Sampark" || skill.name == "Emcee"){
              candidate.participatingSkills.push({name: skill.name, duration: skill.duration, color: skill.color });
              break;
            }
            else {
              if(candidate.participatingSkills.filter(c=>c.name == "Ghanshyam" || c.name == "Neelkanth" || c.name == "Sahajanand").length == 0){
                candidate.participatingSkills.push({name: skill.name, duration: skill.duration, color: skill.color });
                break;
              }
            }
    ...
    // sort candidate.participatingSkills
    // for (let index = 1; index <= length; index++) {
    //   this.candidates[index].participatingSkills.sort((a, b) => {
    //     if (a.sliceStart! < b.sliceStart!) return -1;
    //     if (a.sliceStart! > b.sliceStart!) return 1;
    //     return 0;
    //   });
    // }
    ...
    //   candidate.participatingSkills = candidate.participatingSkills
    //   .sort((a, b) => a.sliceStart!.localeCompare(b.sliceStart!))
    // });

    //console.log(this.input, this.candidates);

    //this.candidatesChanges.emit(this.candidates);
  }
  */
}